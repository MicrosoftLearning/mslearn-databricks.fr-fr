...
5. Répétez les étapes ci-dessus pour également installer `databricks-vectorsearch==0.40`.
...
4. Dans une nouvelle cellule, exécutez le code suivant pour créer un dataframe à partir des données brutes :
...
5. Dans une nouvelle cellule, exécutez le code suivant, en remplaçant `<catalog_name>` par le nom de votre catalogue Unity (le catalogue avec le nom de votre espace de travail et un suffixe unique), pour nettoyer et prétraiter les données afin d’extraire les champs de texte appropriés :
...
## Augmenter les invites avec des données récupérées :
